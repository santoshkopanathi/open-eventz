...
**E2E follow-up (post-merge fix).** The reskin was pushed to master and **the CI Playwright smoke suite went red** — E2E is CI-only (not in the pre-push hook, which runs typecheck + unit), so the local gate was green while the pipeline wasn't. Three assertions were coupled to presentation the reskin changed: the `↻ Recurring` card label (now plain `Recurring`), the confirmed-family chip background (`#F5F0DE` → `#F3EDE3`), and a `button.rounded-full` selector on the filter dropdown (I'd moved that radius to an inline style — restored the class). Fixed the two assertions + the class, re-ran locally (9/9 green), pushed. **Lesson (Concept E, again): a green pre-push hook is not a green pipeline — for UI changes, run `npx playwright test` locally before pushing, since the hook deliberately excludes E2E.**
...
**Post-reskin polish (user feedback, reviewed via a before/after mockup first).** Four small tweaks after looking at the live reskin: (a) **taller date box** so the month/day sit centred instead of hugging the top; (b) **time larger (13px) + darker (`ink-70`) + centre-aligned** with the badge row — it was `ink-35` 11px and top-aligned, too faint to read on mobile; (b-extra) **Registration/Recurring badges collapse to line-icons on mobile** (clipboard / repeat), full text on desktop — *why:* this responsive collapse existed pre-reskin (as `📋`/`↻`) and my emoji purge silently dropped it, so it's restored emoji-free **and given its own E2E test** (§11.5) so it can't regress unnoticed a second time; (c) **removed the "Frisco City / Plano City" title** inside the filter box (redundant — the active city is already the black segmented tab above); (d) **highlighted the filters** — the dropdown pills were transparent and the Today/Tomorrow/Weekend presets were the *same* `fill-subtle` as the box they sat in, so both vanished; they now use a paper fill + firm `border-strong` (active preset = solid rust) so they read unmistakably as buttons. Verified: typecheck + 233 unit + **10 E2E** + `next build`, all green.

---

## Filter-bar + consent-banner mobile polish

**Date:** 2026-08-07 · **Deployed** (`bf51252`, pushed to `master` → Vercel).

**Initial situation.** On mobile the filter card had two rough edges the user flagged from a live screenshot: (1) the **"Show map"** toggle read as an oversized `text-sm` control; (2) the manual date range (`Sources · Age range · From [date] to [date]`) all sat in **one `flex-wrap` row**, so the two wide native date inputs wrapped wherever they ran out of room — leaving a lonely second date (`Aug 13, 2026`) stranded on its own line. Separately, the **analytics consent banner** was a tall 4-line block (`fixed bottom-0`) that covered ~40% of a phone screen and overlapped the last event cards.

**What changed — and the reasoning:**

- **"Show map" smaller.** `text-sm px-3.5 py-2` → `text-xs px-3 py-1.5`. It was already right-aligned (the tabs row is `justify-between`); only the weight was the issue.
- **Dedicated date row (the anti-wrap fix).** Pulled `From … to …` out of the shared wrap-row onto **its own row** beneath the dropdowns, with the From/to pair wrapped in a single **non-wrapping** `inline-flex` unit — so on a narrow screen the whole range drops together, never split mid-range. *Why this over a single custom range-picker:* native `<input type="date">` is reliable + accessible; the only real problem was layout, so fix the layout, not the control.
- **"Clear filters" only when non-default.** It used to sit **always-visible** on its own top-right row — a no-op in the default state (nothing to clear) and a wasted row. Now `page.tsx` computes `canClear` (any source/branch/age selected, or the date window moved off the default today→+7) and the link renders only then. Gated on `mounted` too, so the SSR and first-client render agree — the date compare uses `new Date()` and would otherwise risk a hydration mismatch. In the common default case the row collapses, shortening the card on mobile. **New E2E test** asserts: hidden by default → appears after selecting an age → disappears on clear.
- **Label cleanups.** Removed the redundant mono **"DATES"** label (the `From` input already labels the row); changed **"JUMP TO"** (mono, uppercased, tracked) to plain **"Jump to"** in the same DM Sans font/size as the `From` label — quieter, consistent.
- **Slim consent banner.** Rewrote the banner from a 4-line stacked block to a **single compact strip** (shorter copy, `text-xs`, row never stacks, buttons stay inline right) so it barely covers the list on mobile. Behaviour unchanged — Consent Mode v2 default-denied, choice remembered in `localStorage`, one-time for undecided visitors. *(Chosen over "reserve scroll space" / "in-flow push" — the user picked the smaller-footprint option.)*
...
## Frisco age filter broke — a client-side-rendering scrape break, fixed with BiblioCommons' own JSON API

**Date:** 2026-08-13. *(Full interview write-up: `08-portfolio/interview-challenges.md` #17. Ingest architecture: `INGEST-DESIGN.md`.)*

**Initial situation.** The Frisco tab began showing **adult events** ("D&D for Adults," "Adult Volunteer Open House"), and the age filters stopped narrowing — "Toddlers (0–5)" returned **305 of 306** events. Prod data confirmed it: **304/306 Frisco events stored as age 0–17**. Frisco age has one source of truth — BiblioCommons' "Suitable for:" audience — and `parseFriscoSuitableFor` was returning null for essentially every event, so all fell to the `0–17` "all ages" fallback, which (a) passes the adults-only gate (`age_min < 18`) and (b) overlaps every kid filter → filter no-op.

**Reason / root cause.** BiblioCommons migrated event pages to a **client-side-rendered `/v2`** app. The "Suitable for:" audience is now hydrated by JavaScript *after* load; our server-side `fetch` gets the raw pre-hydration HTML where `<span itemprop="name">` is **empty** (verified: no JSON-LD, no `__NEXT_DATA__`, no data blob). The field is visible in a browser but not in the bytes we scrape. Aggravator: the new nightly ingest re-scraped the now-empty source and overwrote any residual good ages with `0–17`. *(Honest correction: my earlier "pure staleness, not a parser break" call was incomplete — I'd checked the listing markup, not the detail-page age extraction.)*

**What we were going to do, and what we did instead.** Plan A was to extend our **Play Frisco LLM age inference** to Frisco Library — robust, but a paid inference dependency to rebuild data that still exists. The pivot came from a PM observation — *the value is right there in the browser* — reframing it as "data moved to the client," not "data gone." Drove a headless browser + network tab and found BiblioCommons' **unauthenticated JSON API**: `GET /events/events/{id}?client_scope=events` (header `Accept: application/json`) → `definition.audience_ids`; `/events/event_audiences` → id→name taxonomy (6 stable audiences, same IDs as the original feeds). Validated: Family Story Time → `[Children (0-5)]`, D&D for Adults → `[Adults]`.

**The change (shipped 2026-08-13).** Re-pointed `ingestFriscoLibrary()` from the dead HTML "Suitable for" scrape to the JSON API: fetch the audience taxonomy once/run (`fetchFriscoAudiences`), then per event read `definition.audience_ids` → map to `age_min`/`age_max` via a new pure, unit-tested **`mapFriscoAudienceIds()`** (9 tests: Adults→18–99, Adults+Teens→13–17, Children (0-5)→0–5, unknown/empty→null-fallback, …). Deterministic, free, authoritative; also takes the cleaner API `description` and captures `featured_image_id` (image hook, separate decision). **Verified on prod after re-ingest (340 events, 0 errors):** age distribution now varied (`0-5:183, 6-12:14, 10-13:2, 13-17:19, 0-17:29`), kid-facing Frisco dropped ~305→247 (adults excluded), **0 adult-title leaks**, toddler filter narrows.

**The guardrails (the real lesson — built same change).** The bug passed every test because the suite is logic-only (mocked) and nothing ran on real data post-ingest, and the parser's graceful `0–17` fallback fired 304× with no error. Three guardrails, mapped to those failure layers:
- **Layer 1 (mocks blind to upstream) → live-source canary:** `validate-data.ts` fetches real BiblioCommons events and asserts `audience_ids` still resolve — catches a contract change directly.
- **Layer 2 (graceful degradation hid it) → fallback-rate warning:** `ingestFriscoLibrary` counts resolved-vs-fallback; >50% fallback pushes a run warning (dashboard-visible).
- **Layer 3 (nothing watched real data) → post-ingest data-quality gate:** new `data-quality` job in `ingest.yml` (`needs: ingest`) runs `scripts/validate-data.ts` against the live DB — age variety, **zero adult-title leaks**, toddler-filter-narrows, per-source non-empty, freshness — plus a real-data filter regression, emitting a pass/fail summary to `$GITHUB_STEP_SUMMARY` and exiting red on any violation. Pure checks in `src/lib/data-quality.ts` (unit-tested against a "healthy" and a "the-incident" fixture). *This is the durable fix: a silent source change is now a RED pipeline, not a green ingest over corrupt data.*

**Verification.** typecheck + **250 unit** (+17: mapper + data-quality) + **11 E2E** + `next build` + doc-parity green; `npm run validate` = all checks pass on prod data.

**The lesson.** When a scrape returns empty, first ask *is the data gone, or did it move to client-side rendering?* — a field visible in the browser but absent from `curl` is the tell. Before reaching for an LLM to reconstruct missing data, check whether the site's own front-end pulls it from an API you can call directly. And the deeper one: **logic tests can't catch bad data** — if a pipeline can "succeed" while producing garbage, you need a guardrail pointed at the real output, or the next silent source change goes unnoticed too.

---

## Filter row goes inline on desktop + event images in the detail view

**Date:** 2026-08-13. Two user-requested UI polish items after the Frisco fix.

**#2 — desktop inline filters.** The dropdowns, date range, and quick presets were on three stacked rows. Merged them into **one responsive `flex flex-wrap` row** (`SourceSubFilter`): on desktop the `From/To` range sits inline with `Sources`/`Age range`, and everything wraps to stacked rows only as the width narrows (mobile is unchanged). The `From/to` pair and the `Jump to` label+pills each stay intact as non-wrapping sub-groups so they never split mid-group. Pure layout change.

**#3 — event images (detail view only).** Frisco Library event cards carry an image, but the `/v2` markup dropped the old `cp-event-image` class (the *same* client-render break as the ages), so `thumbnail_url` was `null` for every event. Fixed the extraction to match the BiblioCommons `uploads/images/…` URL directly; re-ingested → **215/247** events now have one. **Encoding gotcha (caught in review):** the card puts `&amp;` and raw spaces in the filename (e.g. `Event Banner Arts &amp; Culture 760x230 V2.jpg`); stored verbatim those URLs 404, so only the clean-named banners (Story Time, Teen Anime) loaded and the rest were `onError`-hidden — looking like "only one banner exists." Fixed by `encodeURI(decodeHtml(url))` at extraction; re-verified all **7 distinct banners return 200**. Rendered as a **hero image at the top of the detail drawer** (`EventDetail`) and the `/events/[id]` server page — **hotlinked** (no storage), `onError`-hidden in the client drawer, omitted entirely when absent, so a missing/broken image never leaves an empty box. **Product note:** these are **category banners** (Story Time / Arts / ESL / Computers), shared across events of a type — decorative, not per-event photos. That's why they're **detail-view only** (one at a time → the repetition is invisible; on a list they'd read as duplicates). `featured_image_id` from the JSON API is captured in `raw_json` as a future upgrade path if per-event images ever appear.

**Mobile detail header consistency (review feedback).** The mobile full-screen detail overlay had its own plain paper header (single-colour serif "Open Eventz") that didn't match the home **ink masthead**. Replaced it with a masthead-matching branding bar (ink `#1F1B16` band + 3px rust rule + two-colour wordmark "Open"/cream · "Eventz"/rust + tagline), so the detail view reads as the same product. (Same review pass also caught the image-URL encoding bug above.)

**Verification.** typecheck + 250 unit + 11 E2E + `next build` + doc-parity green; verified on the dev server — desktop filters inline / mobile stacked, drawer + mobile-overlay hero banner rendering (all 7 banners load) with graceful absence, and the mobile detail header now matching the home masthead (screenshots).

---

## Play Frisco — wider coverage window + event images

**Date:** 2026-08-13. Review feedback: Cultural Affairs (CID=85) events looked under-covered.

**Which calendar / why events were sparse.** Play Frisco ingests `friscotexas.gov/calendar.aspx?CID=85,81` (**Cultural Affairs + Parks & Rec**), then keeps only LLM-classified **kid-relevant free** events (so adult/professional items like "Call for Art" and keyword-excluded "Reception, …" are correctly dropped — that part works; Princess Tea *was* showing). The real gap was the **window**: the EID loop covered 2 months **and** a per-event `startDate > oneMonthFromNow` check hard-capped it at **~1 month**, so only ~3 events showed. **Widened to ~6 months** (loop `i < 6`, `horizon = now + 6mo`). Result on prod: **3 → 14 kid-facing events**, spanning Aug 2026 → Jan 2027 (Frisco Camp Out, Unplug Texas Day, Merry Main Street, Frosty 5K, …).

**Images.** CivicPlus exposes an event image as `og:image` on the detail page (which we already fetch). Extract it into `thumbnail_url` — **but** events without a real image fall back to a generic calendar icon (`/Images/SocialMedia/IconModuleCalendar.png`), which would be an awkward placeholder hero, so we keep only real `/ImageRepository/Document` images and null the rest. Result: **9/14 have a real per-event image**, the other 5 gracefully show no banner. Same detail-view-only render as Frisco Library (already deployed). **Plano:** confirmed **no per-event image source** (no feed enclosure, no `og:image` on 5/5 sampled event pages) → deliberately skipped. Also silenced the "no startDate itemprop" per-EID noise (low EIDs like 1/37/58 are calendar nav links, not events).

**Classifier: LLM-primary (the exclude list was over-fitted).** As the app scales to more sources/cities, a 25-substring hand-tuned `PARKS_REC_EXCLUDE_KEYWORDS` doesn't — it's brittle (substring false positives: "reception," could catch a kids event; "board" would catch "Board Game Night"), high-maintenance, and per-source. But the LLM (`inferPlayFriscoEvent`, Sonnet, cached) **already decides `kid_relevant`** — that's its whole job. So we made the LLM primary and shrank the keyword list to a tiny **governance-only pre-filter** (`city council`, `board meeting`, `work session`, … — multi-word, near-zero false positives) whose only role is to skip city-admin noise we don't want to pay to classify. Everything else — kid vs adult — is the model's call, with a stored `reasoning` and dashboard visibility (vs a silent keyword skip). Made it **fail-closed** for a kids app: an LLM failure or a `low`-confidence result now sets `kid_relevant = false` (uniform pass over cached + fresh), plus a hard post-LLM override for explicitly `adults only / 21+ / 18+`. **Result on prod:** the LLM correctly hides the adult items the user flagged ("Call for Art" → *"targeting adult artists"*, "Reception, …" → *"adult-oriented art reception"*) **and** splits ambiguous ones the keyword list couldn't — "Cycle the City – Greenbelt" (family ride) shown, "Cycle the City – Dominion Trail" (*"civic engagement with elected officials"*) hidden. 21 kid-relevant / 10 hidden of 31 scraped; 0 fail-open nulls.

**Verification.** typecheck + 250 unit + 11 E2E + `next build` + doc-parity green; prod re-ingested (15 kid-facing events Aug 2026→Jan 2027, real images load + vary, adult/professional items correctly hidden by the LLM with auditable reasoning).

---

## New source: Kaleidoscope Park — first run of the onboarding playbook

**Date:** 2026-08-13. *(Playbook: [`SOURCE-ONBOARDING.md`](./SOURCE-ONBOARDING.md); this is the concise log entry.)*

**Why & what.** First expansion beyond the three seed sources — Kaleidoscope Park (Frisco's signature Hall Park venue), family/community programming (SaturYAY!, festivals, concert series). We wrote the **source-onboarding playbook** first, then ran it as the test.

**The find.** WordPress + **The Events Calendar** → a **public JSON REST API** (`/wp-json/tribe/events/v1/events`) with fully-structured data — the *best-case* source type after four scraping battles. Gotcha (now a playbook principle): the bare API is **403 WAF-blocked**; it returns **200** with `Accept: application/json` + a browser UA + `Referer`. UTC times are provided directly (no timezone math); venue geo is often empty → park-coords fallback.

**The build.** `EventSource` + `site.ts`/`supervision.ts` branches (park → "check with venue"), `ingestKaleidoscope()` + `runKaleidoscopeIngest()`, the `scripts/ingest.ts` arg + `ingest.yml` matrix job, a data-quality non-empty check, and the Frisco sub-filter option. Crucially, the shared LLM classifier was **extracted into `classifyEvents()`** (from Play Frisco's inline block) in the same change, so this onboarding *reused* it rather than duplicating — the source was **smaller than the last one, not bigger** (the whole point of a playbook). No migration.

**Result.** **103 events → 84 kid-facing** on prod (Sep 2026 → Feb 2027), all with images. The LLM-primary classifier proved itself on a mixed-audience source: **"Pop & Pour" (a wine event) hidden** — reasoning *"explicitly requires guests to be 21+"* — while family events show. typecheck + 250 unit + 11 E2E + build + doc-parity + `npm run validate` green; Play Frisco re-run = **0 LLM calls** (the classifier extraction is regression-free).

**The lesson.** The second-source cost is where a data product's expansion thesis is proven or disproven. Ours got *cheaper*: a documented playbook + a reused classifier meant adding a whole new source was mostly wiring, and the one novel thing (a WAF-blocked API) became a durable playbook principle instead of a fresh surprise.
