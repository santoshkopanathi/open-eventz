...
/**
 * The Open Eventz app itself. Rendered by `/` (no city preselected — defaults to Frisco)
 * and by the city routes `/frisco` and `/plano`, which preselect their own city so search
 * traffic lands in the working app rather than on a separate stripped-down list page.
 */
export default function EventsApp({ initialCity }: { initialCity?: City } = {}) {
  ...
  const [city, setCity] = useState<City>(initialCity ?? 'frisco')
  ...
  // The city routes pass `initialCity` instead, which already set the state above; re-reading
  // the query string there would only let a stray ?city= override the route's own city.
  useEffect(() => {
