// Run prompt v2 over the 64 labelled golden-set events.
// Reads: eval/_parsed.json (labels), Supabase (descriptions)
// Writes: eval/run-v2-results.json
...
;(async () => {
  ...
  const out = []
  console.log('running prompt v2 over ' + labelled.length + ' events\n')
  ...
  console.log('\n')
  fs.writeFileSync('eval/run-v2-results.json', JSON.stringify(out, null, 2))
  console.log('wrote eval/run-v2-results.json — ' + out.length + ' rows, ' + out.filter(r => r.error).length + ' errors')
})()
