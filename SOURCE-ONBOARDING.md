...
9. **A wrong value is worse than a missing one — so validate BEFORE the write, not after.** Every runner writes through `guardedUpsert`/`screenBatch` (`src/lib/ingest-guard.ts`): implausible events are dropped, and a systemically-wrong batch (uniform time shift, mass implausibility, partial scrape) is **rejected entirely**, leaving the previously-stored correct rows in place. A post-write gate only tells you how long you were wrong. When onboarding a source, confirm its first run passes the guard rather than needing the escape hatch.
10. **When a source teaches you a lesson, audit every OTHER source for the same shape that day.** Principle #7 was written after Kaleidoscope's bad UTC and not applied to the other three sources — which is precisely how the same class of bug shipped again a day later. A principle recorded but not applied backwards is a note, not a control.
...
- **Platform:** WordPress + **The Events Calendar** (`tribe-events`). **Endpoint:** `GET /wp-json/tribe/events/v1/events` (needs `Accept: application/json` + browser UA + `Referer` — a bare request is **403 WAF-blocked**). Structured: title, dates, venue (geo often empty → park-coords fallback), `cost`, `image.url`, description. **Timezone gotcha (Principle 7):** the site's `utc_*` is 10h wrong (WordPress TZ = "UTC+5"), so we convert the local `start_date` as America/Chicago (`centralWallTimeToUtc`).
