...
| **Fire drill** | **The first drill (2026-08-17) failed — and that is the entry's whole justification.** `notify` died with a 503 while creating its label, *before* it ever tried to open the issue: a cosmetic step took down the alert. Now every call retries on 5xx, labelling is best-effort and off the critical path, issue creation falls back to no-label, and total failure is reported loudly so GitHub's own workflow-failure email (which *did* arrive) still lands. Verified against mocked 503/422 paths before re-running. Because `notify` only fires on failure, it would otherwise sit unproven until the night it matters. Run the **Ingest events** workflow manually with **`simulate_failure` ticked**: one job fails on purpose, the real ingest step is skipped for *every* source (nothing scraped, classified or written), and the alert fires under the separate `ingest-drill` label so a lingering test issue can never absorb a real alert. **If the drill issue reaches your inbox, the whole chain works; if you only find it on GitHub, the job works but the email path doesn't** — check `github.com/settings/notifications` and the repo Watch setting. Scheduled runs are unaffected (`inputs` is undefined, so the drill step is skipped). Re-run after any change to the alert. |
